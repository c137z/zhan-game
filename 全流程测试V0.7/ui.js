...
function renderProphetRow() {
  ...
  row.innerHTML = '';
  var flatPiles = G.piles.flat();
  // 只显示5列（一行），每列对应一列牌堆的下层牌
  for (var col = 0; col < CONFIG.BOARD_COLS; col++) {
    // 找到该列中第一摞有下层牌的pile
    var subCard = null;
    for (var rowIdx = 0; rowIdx < CONFIG.BOARD_ROWS; rowIdx++) {
      var pile = G.piles[rowIdx][col];
      if (pile.length > 1) { subCard = pile[pile.length-2]; break; }
    }
    var chip = document.createElement('div');
    chip.className = 'prophet-chip';
    if (subCard) {
      var sct = CARD_TYPES[subCard.type] || { emoji: '?' };
...
(function() {
  ...
  var timer;
  el.addEventListener('touchstart', function(e) {
    timer = setTimeout(function() { showPlayerRelicInfo(); }, 600);
  }, {passive: true});
  el.addEventListener('touchend', function() { clearTimeout(timer); });
  el.addEventListener('touchmove', function() { clearTimeout(timer); });
...
function showPlayerRelicInfo() {
  ...
  document.getElementById('boss-info-emoji').textContent = '🦸';
  document.getElementById('boss-info-name').textContent = '勇者圣物';
  document.getElementById('boss-info-mechanic').textContent = desc;
  document.getElementById('boss-info-stats').textContent =
